...
all_rows = []    
csv_path = os.path.join("output", "ldm2.0", "eval_table.csv")
...
def compute_topk_time_consistent(pred_samples, gt_seq, metric_fn, occ_thr=0.5, reduce="mean"):
    ...
    """
    T, K = pred_samples.shape[0], pred_samples.shape[1]
    sample_scores = []
    per_sample_per_t = []
    ...
    for k in range(K):
        per_t = []
        for t in range(T):
            s = metric_fn(pred_samples[t, k], gt_seq[t], occ_thr=occ_thr)
            per_t.append(s)
...
def save_gt_maps(x_gt, output_dir, batch_idx, seq_len=SEQ_LEN):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"mask_{batch_idx}.png"), dpi=300)
    plt.close(fig)


def save_pred_mean_maps(prediction_maps, output_dir, batch_idx, seq_len=SEQ_LEN):
    """
    prediction_maps: (T, K, 1, H, W)
    ...
    for m in range(seq_len):
        ...
        a.set_title(f"n={m+1}", fontdict={"fontsize": fontsize})
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"pred_mean_{batch_idx}.png"), dpi=300)
    plt.close(fig)
...
def save_pred_mean_overlay_maps(prediction_maps, x_gt, output_dir, batch_idx, occ_thr=0.5, seq_len=SEQ_LEN):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"pred_mean_overlay_{batch_idx}.png"), dpi=300)
    plt.close(fig)
...
@torch.no_grad()
def evaluate_ldm(model, dataloader, device, output_dir, metric="iou", coverage_tau=0.5, ddim_steps=10, ddim_eta=1.0, 
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="IoU threshold for Coverage",
    )
    parser.add_argument(
        "--ddim_steps",
        type=int,
