...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        logvar = torch.full((self.num_timesteps,), fill_value=logvar_init, dtype=torch.float32)
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_cond_stage(self, config):
        if not self.cond_stage_trainable:
            if config == "__is_first_stage__":
                ...
                self.cond_stage_model.train = types.MethodType(disabled_train, self.cond_stage_model)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            #     print(f"[DEBUG apply_model] x_noisy range: [{x_noisy.min():.4f}, {x_noisy.max():.4f}]")
            #     print(f"[DEBUG apply_model] x_recon range: [{x_recon.min():.4f}, {x_recon.max():.4f}]")
...
class PredOccLatentDiffusion(LatentDiffusion):
    ...
    def load_first_stage(self, path):
        sd = torch.load(path, map_location="cpu")
        if "state_dict" in sd:
            sd = sd["state_dict"]
        ...
        for k, v in sd.items():
            if k.startswith("first_stage_model."):
                new_sd[k[len("first_stage_model."):]] = v
            else:
                new_sd[k] = v

        missing, unexpected = self.first_stage_model.load_state_dict(new_sd, strict=False)
        print(f"[first_stage] loaded from {path}")
        print(f"[first_stage] missing: {missing}")
        print(f"[first_stage] unexpected: {unexpected}")
    ...
    def p_losses(self, z_full, z_masked, m, t, noise=None):
        ...
        if noise is None:
            ...
            # add noise only to unknown parts (future frames)
            future_mask = (m == 0).float()
            noise = noise + future_mask * torch.randn_like(z_full)
        x_noisy = self.q_sample(x_start=z_full, t=t, noise=noise)
        
        # fold time -> channel for 2D UNet
        ...
        target_2d = rearrange(target, 'b t c h w -> b (t c) h w')   # (B, 2T*C, H, W)
       
        # implement loss on only unknown parts (future frames)
        # m: past=1, future=0  -> unknown = 1-m

        unknown_mask = 1.0 - m                                          # (B, 2T, 1, H, W)
        unknown_mask = unknown_mask.expand(-1, -1, z_full.shape[2], -1, -1)
        ...
        denom = unknown_mask_2d.sum(dim=(1, 2, 3)).clamp(min=1.0)
        loss_simple = loss_simple_raw.sum(dim=(1, 2, 3)) / (denom + 1e-6)
        loss_dict.update({f"{prefix}/loss_simple": loss_simple.mean()})
        ...
        loss_vlb = loss_simple_raw.sum(dim=(1, 2, 3)) / denom
        loss_vlb = (self.lvlb_weights[t] * loss_vlb).mean()
        loss_dict.update({f'{prefix}/loss_vlb': loss_vlb})
