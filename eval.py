...
@torch.no_grad()
def evaluate_ldm(model, dataloader, device, output_dir, ddim_steps=10, ddim_eta=0.0, 
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Evaluating", total=num_batches)):
            ...
            for m in range(SEQ_LEN):   
                ...
                plt.yticks([])
                a.set_title(f"n={m+1}", fontdict={'fontsize': fontsize})
            img_path = os.path.join(output_dir, f"pred_{batch_idx}.png")
            plt.savefig(img_path, dpi=500)
            plt.close(fig)

            overlay_gif_name = os.path.join(output_dir, f"pred_gif_{batch_idx}.gif")
