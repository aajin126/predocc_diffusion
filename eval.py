...
def evaluate_ldm(model, dataloader, device, output_dir, ddim_steps=20, ddim_eta=1.0, 
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Evaluating", total=num_batches)):
            ...
            x_in, x_gt, _, x_rel, y_rel, th_rel = model.get_input(
                                                            batch,
                                                            process = 'test'
                                                        )
            ...
            c = {
                "c_spade": c["c_spade"].repeat_interleave(num_samples, dim=0),
            }
            ...
            with model.ema_scope("Evaluation"):
                z_samples_stacked, _ = model.sample_log(
                    cond=c,
            ...
            z_samples = model._unstack_time_channel_latent(z_samples_stacked) # (B_vis*K*T, embed_dim, 16, 16)
