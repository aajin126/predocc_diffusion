...
from predocc.data.data_preprocessing import preprocess_batch
...
class ImageLogger(Callback):
    ...
    def _wandb(self, pl_module, images, batch_idx, split):
        ...
        logged_images = {}
        for k in images:
            if isinstance(images[k], plt.Figure):
                logged_images[f"{split}/{k}"] = wandb.Image(images[k])
            else:
                # Handle torch tensor (grid)
                grid = images[k]
                grid = torchvision.utils.make_grid(grid, nrow=10)
                logged_images[f"{split}/{k}"] = wandb.Image(grid)
        pl_module.logger.experiment.log(logged_images, step=pl_module.global_step)
    ...
    def log_local(self, save_dir, split, images,
        ...
        for k in images:
            if isinstance(images[k], plt.Figure):
                ...
                images[k].savefig(path, dpi=600, bbox_inches='tight')
            else:
                grid = torchvision.utils.make_grid(images[k], nrow=10)
...
if __name__ == "__main__":
    ...
    if opt.resume: # Resume training from the previous checkpoint.
        ...
        _tmp = logdir.split("/")
        nowname = _tmp[-1]
    else: # New training run. Create new log directory.
        if opt.name:
            name = "_" + opt.name
        elif opt.base:
    ...
    try:
        ...
        if "devices" not in trainer_config:
            trainer_config["devices"] = 1 if torch.cuda.is_available() else 1
        ...
        if not cpu:
            print(f"Running on accelerator={trainer_config['accelerator']}, devices={trainer_config['devices']}")
        ...
        default_logger_cfgs = {
            "wandb": {
                ...
                "params": {
                    ...
                    "offline": opt.debug,
                    "id": nowname,
                    "project": opt.project if opt.project else "predOcc",
                }
            },
        }
        ...
        if 'ignore_keys_callback' in callbacks_cfg and 'resume_from_checkpoint' in trainer_config:
        ...
        if not cpu:
            ...
            if isinstance(devices, int):
                ...
                if devices == "auto":
                    ...
                    ngpu = len([d for d in devices.split(",") if d.strip() != ""])
            else:
                ngpu = 1
        else:
            ngpu = 1
